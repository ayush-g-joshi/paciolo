...
class EditPaymentExtraCostScreen extends StatefulWidget {
  bool isPositive;
  var id;
  EditPaymentExtraCostScreen({Key? key, required this.isPositive, this.id}) : super(key: key);

  @override
  State<EditPaymentExtraCostScreen> createState() => _EditPaymentExtraCostScreenState();
}

class _EditPaymentExtraCostScreenState extends State<EditPaymentExtraCostScreen> implements ResponseListener {

  String TAG = "_EditPaymentExtraCostScreenState";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        Text(
          Constant.LANG == Constant.EN ? ENG.EDIT_PAYMENT_REGISTRY_POSITIVE_EXTRA_COST : IT.EDIT_PAYMENT_REGISTRY_POSITIVE_EXTRA_COST,
        ...
        Text(
          Constant.LANG == Constant.EN ? ENG.EDIT_PAYMENT_REGISTRY_NEGATIVE_EXTRA_COST : IT.EDIT_PAYMENT_REGISTRY_NEGATIVE_EXTRA_COST,
          style: gothamRegular.copyWith(
            fontSize: Dimensions.FONT_L,
            color: Colors.white,
          ),
      ...
      decoration: InputDecoration(
        suffixIcon: Container(
          decoration: BoxDecoration(
              border: Border.all(
                  color: widget.isPositive == true ? const Color(AllColors.colorGreen) : const Color(AllColors.colorRed)
              ),
    ...
    return TextFormField(
      ...
      onTap: () async {
        filterSubjectResult = await Navigator.push(context, MaterialPageRoute(
          builder: (context) {
            return const SelectAssociateSubjectExtraCostScreen();
          },
        ));
    ...
    return TextFormField(
      ...
      onTap: () async {
        if(selectedSubjectId == null) {
          ...
          filterDocumentResult = await Navigator.push(context, MaterialPageRoute(builder: (context) {
              return SelectAssociateDocumentExtraCostScreen(categoryId: selectedSubjectId,);
            },
          ));
  ...
  void getPaymentData() {
    // https://devapi.paciolo.it/account/transaction-extra-cost/900
    setState(() {
      showLoader = true;
    });
    GetRequest request = GetRequest();
    request.getResponse(
        cmd: "${RequestCmd.getPaymentExtraCost}/${widget.id}",
  ...
  void onSuccess(response, responseCode) {
    if(responseCode == GET_PAYMENT_DATA) {
      ...
      setState(() {
        ...
        if(paymentData["document_name"] != null && paymentData["document_name"] != "") {
          ...
          selectedDocumentValue = paymentData["document_name"];
        }
    ...
    } else if (responseCode == GET_CATEGORY_DATA) {
      setState(() {
        showLoader = false;
        debugPrint("$TAG GET_CATEGORY_DATA ========> ${response[Constant.data]}");
        var list = response[Constant.data];
        for(int i = 0; i < list.length; i++) {
          if(paymentData["customer_id"] == list[i]["id"]) {
            selectedSubjectId = list[i]["id"];
            selectedSubjectValue = list[i]["name"];
            break;
          }
        }
        ...
        for(int i = 0; i < list.length; i++) {
          ...
          if(subs != null && subs.length > 0) {
            for(int j = 0; j < subs.length; j++) {
              if(paymentData["customer_id"] == subs[j]["id"]) {
                selectedSubjectId = subs[j]["id"];
                selectedSubjectValue = subs[j]["name"];
                break;
              }
            }
          }
        }
        subjectController.text = selectedSubjectValue.toString();
      ...
      setState(() {
        debugPrint("$TAG SAVE_PAYMENT_DATA ========> ${response[Constant.data]}");
        showLoader = false;
  ...
  void savePaymentTransaction() {
    ...
    request.getResponse(
      cmd: "${RequestCmd.savePaymentCategoryCost}/${widget.id}",
      token: userModel!.authorization,
      body: body,
